                               ...
                               )
{
    ...
    u32TimeoutTicks = OsIf_MicrosToTicks(200, (OsIf_CounterType)QSPI_IP_TIMEOUT_TYPE);
    u32CurrentTicks = OsIf_GetCounter((OsIf_CounterType)QSPI_IP_TIMEOUT_TYPE);
                                  ...
                                  )
{
    ...
    else
    {
        ...
        if (STATUS_QSPI_IP_SUCCESS == status)
        {
            ...
            writeAddress = Qspi_Ip_CalcPaddingWrite(state, address, size);

            status = Qspi_Ip_RunWriteCommand(instance, state->configuration->writeLut, writeAddress, data, size);
            if (STATUS_QSPI_IP_SUCCESS == status)
            {
                               ...
                               )
{
    ...
    if (pConfig->ctrlAutoCfgPtr != NULL_PTR)
    {
        ...
        printf("Qspi_Ip_ControllerInit %d\n", status);
    }
    if (STATUS_QSPI_IP_SUCCESS == status)
    {
       /* Perform initial reset */
        ...
        printf("Qspi_Ip_InitReset %d\n", status);
    }
    if (STATUS_QSPI_IP_SUCCESS == status)
    {
        /* Execute initial setup of external device */
		status = Qspi_Ip_InitDevice(instance, state);
		printf("Qspi_Ip_InitDevice %d\n", status);
    }
    ...
    if (STATUS_QSPI_IP_SUCCESS == status)
    {
        ...
        printf("Qspi_Ip_InitProtection %d\n", status);
    }
