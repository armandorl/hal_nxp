...
#define UART_AR_RELEASE_MINOR_VERSION_C       4
#define UART_AR_RELEASE_REVISION_VERSION_C    0
#define UART_SW_MAJOR_VERSION_C               4
...
/* Check if current file and Uart header file are of the same Autosar version */
    ...
    #error "Uart.c and Uart.h have different vendor ids"
#endif
    ...
    #error "AutoSar Version Numbers of Uart.c and Uart.h are different"
#endif
/* Check if current file and Uart header file are of the same Software version */
    ...
    #error "Software Version Numbers of Uart.c and Uart.h are different"
#endif
        ...
        /* Check if the source file and Det header file are of the same Autosar version */
            ...
            #error "AutoSar Version Numbers of Uart.c and Det.h are different"
        #endif
    #endif
...
#define UART_START_SEC_VAR_CLEARED_32
#include "Uart_MemMap.h"
...
#define UART_STOP_SEC_VAR_CLEARED_32
#include "Uart_MemMap.h"
/*==================================================================================================
                                       GLOBAL CONSTANTS
==================================================================================================*/
...
static Std_ReturnType Uart_StartSyncReceive(uint8 CoreId, uint8 Channel, uint8* Buffer, uint32 BufferSize, uint32 Timeout);
static Std_ReturnType Uart_StartSyncSend(uint8 CoreId, uint8 Channel, const uint8* Buffer, uint32 BufferSize, uint32 Timeout);
static Std_ReturnType Uart_StartAsyncReceive(uint8 CoreId, uint8 Channel, uint8* Buffer, uint32 BufferSize);
static Std_ReturnType Uart_StartAsyncSend(uint8 CoreId, uint8 Channel, const uint8* Buffer, uint32 BufferSize);
static void Uart_StartSetBuffer(uint8 CoreId, uint8 Channel, uint8* Buffer, uint32 BufferSize, Uart_DataDirectionType Direction);
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                TempReturn = Uart_Ipw_SyncReceive(Channel,
                                                  Buffer,
                                                  BufferSize,
                                                  Timeout);
                (void)CoreId;
...
static Std_ReturnType Uart_StartAsyncReceive(uint8 CoreId, uint8 Channel, uint8* Buffer, uint32 BufferSize)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                TempReturn = Uart_Ipw_AsyncReceive(Channel,
                                                   Buffer,
                                                   BufferSize);
                (void)CoreId;
            }
...
void Uart_Init(const Uart_ConfigType * Config)
{
    ...
    if (UART_DRV_UNINIT != Uart_au8DriverStatus[CoreId])
    {
        #if (UART_DEV_ERROR_DETECT == STD_ON)
    ...
    else
    {
        ...
        #if (UART_MULTICORE_SUPPORT == STD_ON)
        else if ((NULL_PTR == Uart_pxPBcfgVariantPredefined[CoreId]) ||
                 (CoreId != Uart_pxPBcfgVariantPredefined[CoreId]->PartitionCoreId))
        {
            #if (UART_DEV_ERROR_DETECT == STD_ON)
            /* Invalid CoreId */
            (void)Det_ReportError((uint16)UART_MODULE_ID,
        ...
        {
            Uart_apConfig[CoreId] = Uart_pxPBcfgVariantPredefined[CoreId];
            (void)Config;
            ...
            {
                if (Uart_apConfig[CoreId]->Configs[u8ChLoop] != NULL_PTR)
                {
                    #if (UART_MULTICORE_SUPPORT == STD_ON)
                    if (CoreId == Uart_apConfig[CoreId]->Configs[u8ChLoop]->ChannelCoreId)
                    {
                    #endif /* UART_MULTICORE_SUPPORT == STD_ON */
                        Channel = Uart_apConfig[CoreId]->Configs[u8ChLoop]->UartChannelId;
                        Uart_Ipw_Init(Channel, Uart_apConfig[CoreId]->Configs[u8ChLoop]->UartChannelConfig);
                        Uart_au32ClockFrequency[CoreId][Channel] = Uart_apConfig[CoreId]->Configs[u8ChLoop]->ChannelClockFrequency;
                    #if (UART_MULTICORE_SUPPORT == STD_ON)
                    }
                    #endif /* UART_MULTICORE_SUPPORT == STD_ON */
                }
            ...
            Uart_au8DriverStatus[CoreId] = UART_DRV_INIT;
        }
    }
}
...
void Uart_Deinit(void)
{
    ...
    /* Check whether the UART driver is in UART_DRV_UNINIT state */
    if (UART_DRV_INIT != Uart_au8DriverStatus[CoreId])
    {
        #if (UART_DEV_ERROR_DETECT == STD_ON)
        /* Uart driver has been already initialized */
        (void)Det_ReportError((uint16)UART_MODULE_ID,
    ...
    else
    {
        ...
        if (IsAllChannelsIdle)
        {
            ...
            Uart_au8DriverStatus[CoreId] = UART_DRV_UNINIT;
            Uart_apConfig[CoreId] = NULL_PTR;
        }
        else
        {
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        TempReturn = Uart_Ipw_SetBaudrate(Channel, Baudrate, Uart_au32ClockFrequency[CoreId][Channel]);
                    }
