...
#define LINFLEXD_UART_IP_HWACCESS_SW_MAJOR_VERSION             4
...
{
    Base->UARTCR |= LINFLEXD_UARTCR_MIS(Enable ? 1UL : 0UL);
}
...
{
    Base->UARTCR |= LINFLEXD_UARTCR_DTU_PCETX(Enable ? 1UL : 0UL);
}
#endif
...
/**
 * @internal
 * @brief Set UARTSR to Default Value
 *
 * This function Set UARTSR to Default Value.
 *
 * @param Base LINFLEXD Base pointer.
 */
static inline void Linflexd_Uart_Ip_SetUARTSRToDefaultValue(LINFLEXD_Type * Base)
{
    Base->UARTSR = 0x0U;
}
