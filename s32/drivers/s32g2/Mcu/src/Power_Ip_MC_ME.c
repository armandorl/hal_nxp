...
#define POWER_IP_MC_ME_AR_RELEASE_MINOR_VERSION_C       4
#define POWER_IP_MC_ME_AR_RELEASE_REVISION_VERSION_C    0
#define POWER_IP_MC_ME_SW_MAJOR_VERSION_C               4
...
static void Power_Ip_MC_ME_ConfigurePartitionOutputSafe(const Power_Ip_MC_ME_PartitionConfigType * PartitionConfigPtr);
#endif
#endif
...
#endif
#endif
static void Power_Ip_MC_ME_ConfigureCore(const Power_Ip_MC_ME_CoreConfigType * CoreConfigPtr, uint8 PartitionIndex);
static void Power_Ip_MC_ME_ConfigureCorePartition1(const Power_Ip_MC_ME_CoreConfigType * CoreConfigPtr, uint8 PartitionIndex);
...
static inline void Power_Ip_MC_ME_TriggerCofbUpdate(uint8 PartitionIndex)
{
    uint32 TempValue = 0U;
    TempValue = Power_Ip_pxMC_ME->McMePrtnArray[PartitionIndex].PRTN_PUPD;
    TempValue &= (~MC_ME_PRTN0_PUPD_PCUD_MASK);
    TempValue |= (MC_ME_PRTNX_PUPD_PCUD_TRIG_U32 & MC_ME_PRTN0_PUPD_PCUD_MASK);
    Power_Ip_pxMC_ME->McMePrtnArray[PartitionIndex].PRTN_PUPD = TempValue;
...
static void Power_Ip_MC_ME_ConfigureCOFB(const Power_Ip_MC_ME_CofbConfigType * CofbConfigPtr, uint8 PartitionIndex)
{
    ...
    Power_Ip_pxMC_ME->McMePrtnArray[PartitionIndex].PRTN_COFB_CLKEN[CofbIndex] = PeripheralsConfig;
...
static void Power_Ip_MC_ME_ConfigureCore(const Power_Ip_MC_ME_CoreConfigType * CoreConfigPtr, uint8 PartitionIndex)
{
    ...
    /* Configuration of all cores within partition1 need to be finished before enable/disable core, because:
    Cortex-A53 cluster0 core0–3 will be enabled by one bit PRTN1_CORE0_PCONF[CCE].
    Cortex-A53 cluster1 core0–3 will be enabled by one bit PRTN1_CORE2_PCONF[CCE]. */
...
/**
* @brief            This function enable/disable cores within partition1.
* @details          Configures the core within the given partition1.
*
* @param[in]        CoreConfigPtr    Pointer to a core configuration structure
*                   (member of 'Power_Ip_MC_ME_PartitionConfigType' struct).
* @param[in]        PartitionIndex        Index of the partition in which the core resides.
...
void Power_Ip_MC_ME_ConfigCoreCOFBClock(const Power_Ip_MC_ME_ModeConfigType * ModeConfigPtr)
{
    ...
    for (PartitionIndex = 0U; PartitionIndex < POWER_IP_MAX_NUMBER_OF_PARTITIONS; ++PartitionIndex)
    {
        ...
        if ( TRUE == TempPartitionConfig->PartitionUnderMcuControl )
        {
            ...
            Cortex-A53 cluster0 core0–3 will be enabled by one bit PRTN1_CORE0_PCONF[CCE].
            Cortex-A53 cluster1 core0–3 will be enabled by one bit PRTN1_CORE2_PCONF[CCE]. */
            if (1U == TempPartitionConfig->PartitionIndex)
            {
                for (CoreIndex = 0U; CoreIndex < TempPartitionConfig->NumberOfCores; ++CoreIndex)
                {
                    if ( TRUE == (*TempPartitionConfig->ArrayPartitionCoreConfigPtr)[CoreIndex].CoreUnderMcuControl )
                    {
                        Power_Ip_MC_ME_ConfigureCorePartition1(&(*TempPartitionConfig->ArrayPartitionCoreConfigPtr)[CoreIndex], TempPartitionConfig->PartitionIndex);
                    }
                }
            }
        }
...
/**
* @brief            This function Enable Partition Output safe stating.
* @details          This function enable Output safe stating by configuring the MC_ME_PRTNx_PCONF[OSSE] module.
*
* @param[in]        ModeConfigPtr   Pointer to MC_ME configuration structure
*                   (member of 'ModeConfigType' struct).
*
...
void Power_Ip_MC_ME_EnablePartitionOutputSafe(const Power_Ip_MC_ME_ModeConfigType * ModeConfigPtr)
{
    ...
    for (PartitionIndex = 0U; PartitionIndex < POWER_IP_MAX_NUMBER_OF_PARTITIONS; ++PartitionIndex)
    {
        ...
        if ( TRUE == TempPartitionConfig->PartitionUnderMcuControl )
        {
            ...
            {
                /* The partition must be enabled before configuring its COFBs and Cores */
                if (MC_ME_PRTNX_PCONF_OSSE_EN_U32 == (TempPartitionConfig->PartitionPconfRegValue & MC_ME_PRTNX_PCONF_OSSE_MASK_U32))
                {
                    Power_Ip_MC_ME_ConfigurePartitionOutputSafe(TempPartitionConfig);
                }
            }
        }
    }
...
/**
* @brief            This function Disable Partition Output safe stating.
* @details          This function Disable Output safe stating by configuring the MC_ME_PRTNx_PCONF[OSSE] module.
*
* @param[in]        ModeConfigPtr   Pointer to MC_ME configuration structure
*                   (member of 'ModeConfigType' struct).
*
...
void Power_Ip_MC_ME_DisablePartitionOutputSafe(const Power_Ip_MC_ME_ModeConfigType * ModeConfigPtr)
{
    ...
    for (PartitionIndex = 0U; PartitionIndex < POWER_IP_MAX_NUMBER_OF_PARTITIONS; ++PartitionIndex)
    {
        ...
        if ( TRUE == TempPartitionConfig->PartitionUnderMcuControl )
        {
            ...
            {
                /* The partition must be enabled before configuring its COFBs and Cores */
                if (MC_ME_PRTNX_PCONF_OSSE_DIS_U32 == (TempPartitionConfig->PartitionPconfRegValue & MC_ME_PRTNX_PCONF_OSSE_MASK_U32))
                {
                    Power_Ip_MC_ME_ConfigurePartitionOutputSafe(TempPartitionConfig);
                }
            }
