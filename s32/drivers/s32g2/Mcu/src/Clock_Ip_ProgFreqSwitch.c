...
static void Clock_Ip_CgmXPcfsSdurDivcDiveDivs(Clock_Ip_PcfsConfigType const *Config)
{
    ...
    if (Hash[Clock_Ip_au8ClockFeatures[Config->Name][CLOCK_IP_PCFS_INDEX]] != ((((uint32)Config->ClockSourceFrequency) ^ ((uint32)Config->MaxAllowableIDDchange) ^ ((uint32)Config->Name)  ^ ((uint32)Config->SelectorName) ^ ((uint32)Config->StepDuration))))
    {
        Hash[Clock_Ip_au8ClockFeatures[Config->Name][CLOCK_IP_PCFS_INDEX]] = ((((uint32)Config->ClockSourceFrequency) ^ ((uint32)Config->MaxAllowableIDDchange) ^ ((uint32)Config->Name)  ^ ((uint32)Config->SelectorName) ^ ((uint32)Config->StepDuration)));

        Finput = Config->ClockSourceFrequency / CLOCK_IP_DIVIDE_BY_1000000;
    #if defined(CLOCK_IP_HAS_FIRC_CLK)
        Fsafe = Clock_Ip_GetConfiguredFrequencyValue(FIRC_CLK) / CLOCK_IP_DIVIDE_BY_1000000;
    #endif
