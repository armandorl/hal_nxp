...
==================================================================================================*/
#define SCHM_CRC_AR_RELEASE_MAJOR_VERSION_C     4
#define SCHM_CRC_AR_RELEASE_MINOR_VERSION_C     4
#define SCHM_CRC_AR_RELEASE_REVISION_VERSION_C  0
#define SCHM_CRC_SW_MAJOR_VERSION_C             4
#define SCHM_CRC_SW_MINOR_VERSION_C             0
#define SCHM_CRC_SW_PATCH_VERSION_C             0
...
static volatile uint32 msr_CRC_EXCLUSIVE_AREA_00[NUMBER_OF_CORES];
static volatile uint32 reentry_guard_CRC_EXCLUSIVE_AREA_00[NUMBER_OF_CORES];
...
#ifdef MCAL_PLATFORM_ARM
#if (MCAL_PLATFORM_ARM == MCAL_ARM_AARCH64)
ASM_KEYWORD uint32 Crc_schm_read_msr(void)
{
    mrs x0, S3_3_c4_c2_1
}
#elif  (MCAL_PLATFORM_ARM == MCAL_ARM_RARCH)
...
#ifdef MCAL_PLATFORM_S12
    #define Crc_schm_read_msr()  ASM_KEYWORD("tfr ccr, d6")
#else
    #define Crc_schm_read_msr() ASM_KEYWORD("mfmsr r3")
#endif
...
void SchM_Enter_Crc_CRC_EXCLUSIVE_AREA_00(void)
{
    uint32 msr;
    uint32 u32CoreId = (uint32)OsIf_GetCoreID();

    if(0UL == reentry_guard_CRC_EXCLUSIVE_AREA_00[u32CoreId])
    {
#if (defined MCAL_ENABLE_USER_MODE_SUPPORT)
        msr = OsIf_Trusted_Call_Return(Crc_schm_read_msr);
#else
        msr = Crc_schm_read_msr();  /*read MSR (to store interrupts state)*/
        ...
        msr_CRC_EXCLUSIVE_AREA_00[u32CoreId] = msr;
    }
    reentry_guard_CRC_EXCLUSIVE_AREA_00[u32CoreId]++;
}

void SchM_Exit_Crc_CRC_EXCLUSIVE_AREA_00(void)
{
    uint32 u32CoreId = (uint32)OsIf_GetCoreID();

    reentry_guard_CRC_EXCLUSIVE_AREA_00[u32CoreId]--;
    if ((ISR_ON(msr_CRC_EXCLUSIVE_AREA_00[u32CoreId]))&&(0UL == reentry_guard_CRC_EXCLUSIVE_AREA_00[u32CoreId]))         /*if interrupts were enabled*/
    {
...
void SchM_Check_crc(void)
{
    ...
    EU_ASSERT(0UL == reentry_guard_CRC_EXCLUSIVE_AREA_00[u32CoreId]);
    reentry_guard_CRC_EXCLUSIVE_AREA_00[u32CoreId] = 0UL; /*reset reentry_guard_CRC_EXCLUSIVE_AREA_00 for the next test in the suite*/
